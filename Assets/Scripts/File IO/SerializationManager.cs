    ...
    public void SaveDataAsText()
    {
        ...
        using(StreamWriter streamWriter = new StreamWriter(savePath))
        {
            ...
            foreach (SimData sim in simManager.sims)
            {
                ...
                foreach(SimData.Trait t in sim.traits)
                {
                    traits += (int)t + ",";
                    numOfTraits++;
                ...
                string data2 = $"{sim.simName},{sim.age},{numOfTraits},{traits}{sim.simColour.r},{sim.simColour.g},{sim.simColour.b},{sim.simColour.a}";
    ...
    public void LoadDataAsText()
    {
        ...
        using(StreamReader streamReader = new StreamReader(savePath))
        {
            while (!streamReader.EndOfStream)
            {
                ...
                List<SimData.Trait> _traits = new List<SimData.Trait>();

                for (int i = 0; i < int.Parse(simSplit[2]); i++)
                {
                    traits += (SimData.Trait)i + 2 + ",";
                    _traits.Add((SimData.Trait)int.Parse(simSplit[i + 2]));
                }
